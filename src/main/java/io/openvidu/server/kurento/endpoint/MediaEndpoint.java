...
public abstract class MediaEndpoint {
	...
	public boolean isRtspToRtspEndpoint() {
		return EndpointType.RTSP_TO_RTSP_ENDPOINT.equals(this.endpointType);
	}

	public boolean isFileToRtspEndpoint() {
	...
	public RtspToRtspEndpoint getRtspToRtspEndpoint() {
		return rtspToRtspEndpoint;
	}
	...
	public synchronized void createRtspToRtpEndpoint(ServerProperties serverProperties, CountDownLatch endpointLatch) {
		...
		builder.buildAsync(new Continuation<RtspToRtpEndpoint>() {

			@Override
			public void onSuccess(RtspToRtpEndpoint result) throws Exception {
				rtspToRtpEndpoint = result;
				log.trace("EP {}: Created a new RtspToRtspEndpoint", endpointName);
				endpointSubscription = registerElemErrListener(rtspToRtpEndpoint);
				endpointLatch.countDown();
			}
			...
			public void onError(Throwable cause) throws Exception {
				endpointLatch.countDown();
				log.error("EP {}: Failed to create a new RtspToRtspEndpoint", endpointName, cause);
			}
		});
	}

	public synchronized void createRtspToRtspEndpoint(ServerProperties serverProperties, CountDownLatch endpointLatch) {

		RtspToRtspEndpoint.Builder builder = new RtspToRtspEndpoint.Builder(pipeline,
				serverProperties.getRtspUri(), serverProperties.getPort());

		builder.buildAsync(new Continuation<RtspToRtspEndpoint>() {

			@Override
			public void onSuccess(RtspToRtspEndpoint result) throws Exception {
				rtspToRtspEndpoint = result;
				log.trace("EP {}: Created a new RtspToRtspEndpoint", endpointName);
				endpointSubscription = registerElemErrListener(rtspToRtspEndpoint);
				endpointLatch.countDown();
			}
	...
	public synchronized void createFileToRtspEndpoint(ServerProperties serverProperties, CountDownLatch endpointLatch) {

		FileToRtspEndpoint.Builder builder = new FileToRtspEndpoint.Builder(pipeline,
				serverProperties.getRtspUri(), serverProperties.getPort());

		builder.buildAsync(new Continuation<FileToRtspEndpoint>() {

			@Override
			public void onSuccess(FileToRtspEndpoint result) throws Exception {
				fileToRtspEndpoint = result;
				log.trace("EP {}: Created a new FileToRtspEndpoint", endpointName);
				endpointSubscription = registerElemErrListener(fileToRtspEndpoint);
				endpointLatch.countDown();
			}
