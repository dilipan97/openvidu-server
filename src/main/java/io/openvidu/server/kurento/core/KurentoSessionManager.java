...
public class KurentoSessionManager extends SessionManager {
	...
	/*
	*  Added - dilipan@datakaveri.org
	*  Create RTSP server endpoint and publish to participant
	*  Function 
	*/
	@Override
	public void publishServerVideo(Participant participant, ServerProperties serverProperties, Integer transactionId)
			throws OpenViduException {

		KurentoParticipant kParticipant = (KurentoParticipant) participant;
		KurentoSession kSession = kParticipant.getSession();

		kParticipant.createServerEndpoint(serverProperties, null);
	...
	public Participant publishIpcam(Session session, MediaOptions mediaOptions,
		...
		try {
			...
			*  Added http to support hls video
			*  Value modified
			*/
			Pattern pattern = Pattern.compile("^(file|rtsp|http)://");
	...
	*/
	@Override
	/* Protected by Session.closingLock.readLock */
	public Participant publishServer(Session session, ServerProperties serverProperties) throws Exception {
		final String sessionId = session.getSessionId();
		...
		URL url = null;
		String protocol = null;
		String rtspConnectionId;
		if(!serverProperties.getType().equals("filetortsp")) {
			try {
				Pattern pattern = Pattern.compile("^(file|rtsp)://");
				Matcher matcher = pattern.matcher(serverProperties.getRtspUri());
				if (matcher.find()) {
					protocol = matcher.group(0).replaceAll("://$", "");
				} else {
					throw new MalformedURLException();
				}
				String parsedUrl = serverProperties.getRtspUri().replaceAll("^.*?://", "http://");
				url = new URL(parsedUrl);
			} catch (Exception e) {
				throw new MalformedURLException();
			}
			...
			rtspConnectionId = serverProperties.getType() + "_" + protocol + "_"
						+ RandomStringUtils.randomAlphanumeric(4).toUpperCase() + "_" + url.getHost()
						+ (url.getPort() != -1 ? (":" + url.getPort()) : "") + url.getPath();
		}
		else {
			rtspConnectionId = serverProperties.getType() + "_file_"
					+ RandomStringUtils.randomAlphanumeric(4).toUpperCase() + serverProperties.getRtspUri();
		}
		...
		this.newTokenForInsecureServer(session, token, serverProperties);
		final Token tokenObj = session.consumeToken(token);

		Participant serverParticipant = this.newServerParticipant(session, rtspConnectionId, tokenObj, location,
				serverProperties.getType());

		// Store a "fake" final user for the IpCam connection
		final String finalUserId = rtspConnectionId;
		this.sessionidFinalUsers.get(sessionId).computeIfAbsent(finalUserId, k -> {
			return new FinalUser(finalUserId, sessionId, serverParticipant);
		}).addConnectionIfAbsent(serverParticipant);

		// Join the participant to the session
		this.joinRoom(serverParticipant, sessionId, null);

		// Publish the IpCam stream into the session
		KurentoParticipant kParticipant = (KurentoParticipant) this.getParticipant(rtspConnectionId);
		kParticipant.deleteIpcamProperties();

		this.publishServerVideo(kParticipant, serverProperties, null);
		return kParticipant;
	}
