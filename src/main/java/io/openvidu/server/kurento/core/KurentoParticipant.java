...
public class KurentoParticipant extends Participant {
	...
	public void createServerEndpoint(ServerProperties serverProperties, String streamId) {
		...
		switch (serverProperties.getType()) {
			case "rtsptortp":
				publisher.createRtspToRtpEndpoint(serverProperties, publisherLatch);
				break;
			case "rtsptortsp":
				publisher.createRtspToRtspEndpoint(serverProperties, publisherLatch);
				break;
			case "filetortsp":
	...
	private void releasePublisherEndpointAux(EndReason reason, Long kmsDisconnectionTime) {
		try {
			...
			else if(publisher.getRtspToRtpEndpoint() != null) {
				mediaElement = publisher.getRtspToRtpEndpoint();
			}
			...
			releaseElement(getParticipantPublicId(), mediaElement);
